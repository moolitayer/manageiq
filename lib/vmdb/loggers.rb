...
module Vmdb
  ...
  module Loggers
    ...
    def self.apply_config(config)
      ...
      apply_config_value(config, $dw_log,     :level_dw,     :level_mw_in_evm)
    ...
    def self.create_loggers
      ...
      if false && ENV.key?("CI")
        ...
        $aws_log   = $kube_log = $mw_log = $dw_log = $scvmm_log = $api_log = $miq_ae_logger = $websocket_log = Vmdb.null_logger
        ...
        $dw_log        = MirroredLogger.new(path_dir.join("datawarehouse.log"), "<DATAWAREHOUSE> ")
