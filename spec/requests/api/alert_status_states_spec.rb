RSpec.describe "Alert Status States API" do
  ...
  let(:miq_alert_status_state) do
    FactoryGirl.create(
      ...
      :miq_alert_status => miq_alert_status,
      :comment          => "Big problem")
  end
  ...
  it "can delete a alert status state through its nested URI" do
    ...
    expect do
      run_post(
        "#{alert_statuses_url(miq_alert_status.id)}/alert_status_states/#{miq_alert_status_state.id}",
        gen_request(:delete)
      )
  ...
  it "can add a alert status state through its nested URI" do
    ...
    expect(response.parsed_body)
      .to include(
        "miq_alert_status_id" => miq_alert_status.id,
        "comment"             => "I was worng",
        "action"              => "comment")
  end
  ...
  it "can add a alert status state through its nested URI" do
    ...
    expect(response.parsed_body["results"].first)
      .to include(
        "miq_alert_status_id" => miq_alert_status.id,
        "comment"             => "I was worng",
        "action"              => "comment")
